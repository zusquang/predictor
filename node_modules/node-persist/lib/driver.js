...
var Driver = Class.extend({
  ...
  getUpdateSql: function(obj) {
    ...
    for (columnKey in obj._getModel().columns) {
      ...
      if (columnKey === "lockVersion") {
        var lockVersionValue = obj[columnKey] + 1;
        columnNamesSql.push(this.escapeColumnName(column.dbColumnName) + ' = ' + lockVersionValue);
      } else {
    ...
    if (typeof obj.lockVersion !== "undefined" && obj.lockVersion !== null) {
      values.push(obj.lockVersion);
      defaultUpdateQuery = 'UPDATE %s SET %s WHERE %s = %s AND %s = %s';
    }

    var sql = util.format(defaultUpdateQuery, obj._getModel().tableName, columnNamesSql.join(','), idColumnName, this.getValuesSubstitutionString(valueSubstitutionIndex++));
    if (typeof obj.lockVersion !== "undefined" && obj.lockVersion !== null) {
      sql = util.format(defaultUpdateQuery, obj._getModel().tableName, columnNamesSql.join(','), idColumnName, this.getValuesSubstitutionString(valueSubstitutionIndex++), 'lock_version', this.getValuesSubstitutionString(valueSubstitutionIndex++));
    }
