...
exports.defineAutoSane = function(name, info, options, callback) {
  ...
  if (info.tables.hasOwnProperty(pluralName)) {
    ...
    if (options.camelise === true) {
      var propertyName, newColumnDefs = {};
      for (propertyName in columnDefs) {
        newColumnDefs[inflection.camelize(propertyName, true)] = persistUtil.shallowCopy(columnDefs[propertyName])
      }
...
exports.connect = function() {
  ...
  if (opts.pooling && opts.pooling.name) {
    ...
    var connectionPriority = opts.connectionPriority || 0;
    return pool.acquire(connectAfterAutoDefinesComplete.bind(this, callback), connectionPriority);
  } else {
